...
def tiene_A(mano):
    ...
    if mano:  # Si mano no está vacía
        return tiene_A(mano[1:])  # Llama recursivamente con el resto de la lista
    return False
...
def ganador(manoJugador, manoDealer):
    print("---------------\nValor de mano final jugador: " , manoJugador , "\nValor de mano final dealer: ", manoDealer)
    if manoDealer==21:
...
def jugar21(baraja, manoJugador, manoDealer):
    ganador(pedirCartaJug(manoJugador,baraja),pedirCartaDealer(manoDealer,baraja))
